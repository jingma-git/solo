...
__global__ void SigmoidFocalLossForward(const int nthreads,
  ...
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
    int n = i / num_classes; // sample idx [0-num)
    int d = i % num_classes;  // current class[0~79];
    int t = targets[n];       // target class [1~80];

    // Decide it is positive or negative case.
    scalar_t c1 = (t == (d + 1)); // when c1==1, positive
    scalar_t c2 = (t >= 0 & t != (d + 1)); // when c2==1, negative

    scalar_t zn = (1.0 - alpha); //coefficient for negative sample
    scalar_t zp = (alpha); // coefficient for positive sample
